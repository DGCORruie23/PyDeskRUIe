...
from src.db.dao.RegistroConteoRDao import update as updateCR
from src.db.dao.PaisDao import getAll as getAllPais
from src.db.models import DatosConteoR
from ttkwidgets.autocomplete import AutocompleteCombobox

class EdicionConteo:
    def __init__(self, root_root, root, opc_paises, datos):
        self.w = 500
        self.h = 700
    ...
    def edicConteoFrame(self):
        ...
        combo_box.set(self.data.nacionalidad)
        ...
        self.ASH_var.set(self.data.AS_hombres)
        self.ASMNE_var.set(self.data.AS_mujeresNoEmb)
        self.ASME_var.set(self.data.AS_mujeresEmb)
        self.MSH_var.set(self.data.NNAsS_hombres)
        self.MSMNE_var.set(self.data.NNAsS_mujeresNoEmb)
        self.MSME_var.set(self.data.NNAsS_mujeresEmb)

        self.nf_var.set(self.data.Nucleos_Familiares)

        self.AAH_var.set(self.data.AA_hombres)
        self.AAMNE_var.set(self.data.AA_mujeresNoEmb)
        self.AAME_var.set(self.data.AA_mujeresEmb)
        self.MAH_var.set(self.data.NNAsA_hombres)
        self.MAMNE_var.set(self.data.NNAsA_mujeresNoEmb)
        self.MAME_var.set(self.data.NNAsA_mujeresEmb)
        ...
        botonG = tk.Button(self.registro_frame, text="Guardar", bg=rojo, fg="white", font=("Arial", 14), command=self.updateC)
    ...
    def updateC(self):
        ...
        registro = DatosConteoR(
            idRegistro= self.data.idRegistro,
